...
/* يحوّل روابط يوتيوب المختلفة إلى رابط embed صالح */
...
function applySettings(settings){
  ...
  // شعار الموقع (اختياري) — يستبدل الحرف الافتراضي داخل الدائرة
  if(settings.site_logo){
    document.querySelectorAll('.logo-mark').forEach(el => {
      el.innerHTML = `<img src="${settings.site_logo}" alt="شعار الموقع" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`;
    });
  }
...
async function renderHome(){
  ...
  if(featuredEl && featured){
    featuredEl.innerHTML = `
      <a class="featured-post reveal" href="post.html?slug=${encodeURIComponent(featured.slug)}">
        <div class="fp-media" ${featured.cover_image ? `style="background-image:url('${featured.cover_image}');background-size:cover;background-position:center;"` : ''}>
          ${featured.cover_image ? '' : `<span class="fp-mark">${(featured.tag || 'جديد').slice(0,1)}</span>`}
          ${featured.video_url ? '<span class="play-badge">▶</span>' : ''}
        </div>
  ...
  grid.innerHTML = rest.map(p => `
    <a class="post-card reveal" href="post.html?slug=${encodeURIComponent(p.slug)}">
      ${p.cover_image ? `<div class="card-media" style="background-image:url('${p.cover_image}')">${p.video_url ? '<span class="play-badge">▶</span>' : ''}</div>` : ''}
...
async function renderPost(){
  ...
  const mediaHtml = embedUrl
    ? `<div class="post-video"><iframe src="${embedUrl}" title="${post.title}" allowfullscreen loading="lazy"></iframe></div>`
    : (post.cover_image ? `<div class="post-cover" style="background-image:url('${post.cover_image}')"></div>` : '');
